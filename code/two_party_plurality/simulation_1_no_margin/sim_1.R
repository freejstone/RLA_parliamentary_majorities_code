...
## ============================================================
## Two-candidate plurality audit, no-margin variant of simulation_1
## ============================================================

## Usage: Rscript sim_1.R W S N p_alice n_false R output_dir [seed]
...
if (length(args) >= 7) {
  ...
  p_alice     <- as.numeric(args[4])
  n_false     <- as.integer(args[5])
  R           <- as.integer(args[6])
  output_dir  <- args[7]
  seed        <- if (length(args) >= 8) as.integer(args[8]) else 42L
} else {
  W <- 60L; S <- 100L; N_ballots <- 5000L
  p_alice <- 0.52; n_false <- 3L; R <- 1L
  output_dir <- "results"
  seed <- 42L
}
...
seat_specs <- data.frame(
  N       = rep(N_ballots, W),
  p_alice = c(rep(p_alice, W - n_false),
...
## Note: eps is hardcoded to 0 so that the existing plot_*.R scripts
## (which facet on eps) work without modification.
...
## -- Method 1: Naive --
set.seed(seed)
cat(sprintf("[Naive]  p_alice=%.2f  n_false=%d  R=%d  seed=%d\n", p_alice, n_false, R, seed))
...
## -- Method 2: Greedy --
set.seed(seed)
cat(sprintf("[Greedy] p_alice=%.2f  n_false=%d  R=%d  seed=%d\n", p_alice, n_false, R, seed))
...
set.seed(seed)
a_greedy3 <- 3L
cat(sprintf("[Greedy a=%d] p_alice=%.2f  n_false=%d  R=%d  seed=%d\n", a_greedy3, p_alice, n_false, R, seed))
...
set.seed(seed)
cat(sprintf("[Bayesian] p_alice=%.2f  n_false=%d  R=%d  seed=%d\n", p_alice, n_false, R, seed))
...
set.seed(seed)
cat(sprintf("[Greedy Bayesian] p_alice=%.2f  n_false=%d  R=%d  seed=%d\n", p_alice, n_false, R, seed))
...
set.seed(seed)
cat(sprintf("[Greedy Bayesian a=%d] p_alice=%.2f  n_false=%d  R=%d  seed=%d\n",
            a_greedy3, p_alice, n_false, R, seed))
...
set.seed(seed)
cat(sprintf("[Top-r Naive] p_alice=%.2f  n_false=%d  R=%d  seed=%d\n", p_alice, n_false, R, seed))
...
if (n_false == 0L) {
  set.seed(seed)
  cat(sprintf("[Full]   p_alice=%.2f  n_false=%d  R=%d  seed=%d\n", p_alice, n_false, R, seed))
...
outfile <- file.path(output_dir,
                     sprintf("results_W%d_palice%.2f_nfalse%d.rds",
                             W, p_alice, n_false))
